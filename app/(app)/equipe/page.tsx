      ...
      <main className="grid grid-cols-1 xl:grid-cols-2 gap-6 flex-1 min-h-0 overflow-hidden">
        ...
        <div className="flex flex-col gap-4 min-h-0">
          ...
          <Card className="shadow-lg border-2 border-primary/20 bg-gradient-to-br from-card via-primary/5 to-secondary/10 backdrop-blur-sm flex-shrink-0">
            ...
            <CardContent className="space-y-3 p-4">
              ...
              <div className="space-y-2 pt-2">
                ...
                <div className="flex items-center gap-2 text-xl font-bold p-2 rounded-lg bg-gradient-to-r from-secondary/10 to-accent/10 border border-secondary/20">
          ...
          <Card className="flex-1 flex flex-col bg-gradient-to-br from-card via-accent/5 to-primary/5 border-accent/20 shadow-lg min-h-0">
            <CardHeader className="bg-gradient-to-r from-accent/10 to-primary/10 rounded-t-lg pb-4 flex-shrink-0">
              <CardTitle className="flex items-center gap-2 text-accent text-xl">
                <UserPlus className="h-6 w-6" />
                Solicitações para Aprovação
                {pendingRequests.length > 0 && (
                  <span className="bg-gradient-to-r from-accent to-primary text-white text-sm px-3 py-1 rounded-full">
                    {pendingRequests.length}
                  </span>
                )}
              </CardTitle>
              <CardDescription className="text-base">Aprove ou recuse as solicitações para entrar na sua equipe.</CardDescription>
            </CardHeader>
            <CardContent className="flex-1 overflow-y-auto p-6 min-h-0">
              {pendingRequests.length > 0 ? (
                <div className="space-y-4">
                  {pendingRequests.map(request => (
                    <div key={request.id} className="flex items-center justify-between p-4 bg-gradient-to-r from-background/70 to-accent/5 rounded-lg shadow-sm border border-accent/10 hover:border-accent/20 transition-all duration-200">
                      <div className='flex items-center gap-4 flex-1 min-w-0'>
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold text-base truncate">{request.user.name}</p>
                      ...
                      <div className="flex gap-2 flex-shrink-0">
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          className="border-green-500/50 text-green-600 hover:bg-green-500/10 hover:border-green-500 transition-all duration-200"
                ...
                <div className="flex flex-col items-center justify-center h-full text-center py-12">
                  ...
                  <p className="text-muted-foreground text-base">Nenhuma solicitação pendente</p>
                  <p className="text-muted-foreground text-sm mt-2">As solicitações aparecerão aqui quando enviadas</p>
                </div>
              )}
            </CardContent>
          </Card>
