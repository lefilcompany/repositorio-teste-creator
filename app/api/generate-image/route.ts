...
const POST = async (req: NextRequest) => {
  try {
    ...
    const availableCredits = teamCredits?.contentSuggestions || 0;

    if (availableCredits <= 0) {
      return NextResponse.json(
        { error: 'Créditos insuficientes para gerar conteúdo.' },
        { status: 403 }
      );
    }
    ...
    // --- 5. DECREMENTAR CRÉDITOS APÓS SUCESSO ---
    const updatedCredits = {
      ...teamCredits,
      contentSuggestions: Math.max(0, (teamCredits?.contentSuggestions || 0) - 1)
    };
