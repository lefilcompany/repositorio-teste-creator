  ...
  return (
    <div className="w-full">
        ...
        <DialogContent className="sm:max-w-md bg-gradient-to-br from-background to-muted/20 border border-primary/30 shadow-xl">
          ...
          {isUserAdmin && (
            <div className="space-y-4">
              <div className="bg-gradient-to-r from-orange-50 to-red-50 p-5 rounded-xl border border-orange-200/50 shadow-sm">
                <div className="flex items-start gap-3">
                  <div className="p-1.5 bg-orange-400/20 rounded-lg flex-shrink-0 mt-1">
                    <AlertTriangle className="h-4 w-4 text-orange-700" />
                  </div>
                  <div className="min-w-0">
                    <h4 className="font-bold text-orange-800 text-sm mb-2">Administrador</h4>
              ...
              <div className="space-y-3">
                ...
                <Select value={selectedNewAdmin} onValueChange={setSelectedNewAdmin}>
                  <SelectTrigger className="h-11 bg-background border border-primary/30 hover:border-primary/50 rounded-lg text-base">
                    <SelectValue
                      placeholder={
                        loadingMembers
                          ? "Carregando..."
                  ...
                  <SelectContent>
                    {teamMembers.length === 0 ? (
                      <SelectItem value="no-members" disabled className="text-sm">
                        Nenhum membro ativo na equipe
                      </SelectItem>
                    ) : (
                      teamMembers.map((member) => (
                        <SelectItem key={member.id} value={member.id} className="text-sm p-3">
                ...
                {teamMembers.length === 0 && (
                  <p className="text-sm text-muted-foreground bg-muted/30 p-3 rounded-lg">
                    Como não há outros membros, você pode prosseguir.
                  </p>
                )}
              </div>
            </div>
          ...
          <DialogFooter className="flex gap-4 pt-4">
            <Button
              ...
              disabled={isLoading}
              className="flex-1 h-11 border border-muted hover:border-muted-foreground rounded-lg font-medium transition-all text-base"
            >
              Cancelar
            </Button>
            <Button
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent className="sm:max-w-md bg-gradient-to-br from-background to-muted/20 border border-destructive/30 shadow-xl">
          <DialogHeader className="space-y-4">
          ...
          <div className="bg-gradient-to-r from-red-50 to-pink-50 p-5 rounded-xl border border-red-200/50 shadow-sm">
            <div className="flex items-start gap-3">
              <div className="p-1.5 bg-red-400/20 rounded-lg flex-shrink-0 mt-1">
                <AlertTriangle className="h-4 w-4 text-red-700" />
              </div>
              <div className="min-w-0">
                <h4 className="font-bold text-red-800 text-sm mb-2">Aviso Importante</h4>
          ...
          <DialogFooter className="flex gap-4 pt-4">
            ...
            <Button
              ...
              disabled={isLoading || (isUserAdmin && teamMembers.length > 0 && !selectedNewAdmin)}
              className="flex-1 h-11 bg-gradient-to-r from-red-500 to-pink-600 hover:from-red-600 hover:to-pink-700 text-white font-medium rounded-lg shadow-md hover:shadow-lg transition-all text-base"
            >
              {isLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
